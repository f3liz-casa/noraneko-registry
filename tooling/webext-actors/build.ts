...
function writeTsubakiActors(): void {
  for (const entry of Deno.readDirSync(ROOT)) {
    ...
    // 殻は lib(std-actor)から来る。無ければ import 先が無いので、ここで言う
    if (!DEPS.some((d) => d.name === "std-actor")) {
      throw new Error(
        `${entry.name}: 殻(std-actor)が [deps] に無い。drop.toml に std = "<uuid>"(umbrella)を書く`,
      );
    }
    const meta = {
      id: a.id,
...
//
// 殻は lib の std-actor。logic が答える三つの door(setup / start / dispatch)
// を呼んで、返ってきた view を描き、effects を carry out する。殻の bytes は
// その lib に一枚だけあって、ここには入らない(この drop に残るのは、下の宣言だけ)。

import { defineContent, defineParent, type ActorMeta } from "../_shared/defineActor.ts";
import { runTsubakiActor } from "std-actor";
...
// The actor keeps its logic in Tsubaki (wasm/), and it runs in a Worker of this
// actor's own -- one per window, terminated when the actor goes.
//
...
// nsContentSecurityUtils. A worker loaded from resource:// has no CSP, so
// compiling there is an ordinary thing. Measured before it was built on: main
// thread "blocked by CSP", worker ok, with this very runtime.
//
// A plain Worker rather than a ChromeWorker, though wasm compiles in either.
// Started from the parent's chrome, the two globals differ by exactly two names
// -- `ChromeWorker` and `ctypes` -- and `ctypes` is the one property that cannot
// be deleted or overwritten (writable:false, configurable:false). It opens any
// dylib and calls into it, so it is a door out of the browser that no amount of
// pruning can shut. Measured 2026-09-13 on this build: pruning a ChromeWorker's
// global down to an allowlist leaves ["ctypes"]; the same prune in a plain
// Worker leaves []. Everything the runtime needs is in both.
//
...
function tsubakiWorker(a: Actor): string {
  ...
  return `
  #tsubaki() {
    const worker = new Worker("resource://noraneko-builtin/${a.dir}/${OPS_WORKER}", {
      name: "${name} tsubaki",
    });
...
//
// ${actorName(a)}'s logic, in a Worker. See tsubakiWorker() in build.ts for
...
/**
 * 殻を呼ぶ一枚。`runContent` は ctx(io / expose / ops)を組んで hook を呼ぶだけの
 * ところで、**どの drop でも同じ**もの。std-actor があるならそちらから取る --
 * 同じ bytes を drop の数だけ配らずに済むし、入れる人が読むのは自分の drop だけになる。
 * 無いとき(noraneko の built-in actor)は、vendor した _shared/ をそのまま束ねる。
 */
function genContentEntry(a: Actor): string {
  const shell = DEPS.some((d) => d.name === "std-actor") ? '"std-actor"' : '"../../_shared/contentRuntime.ts"';
  return `// SPDX-License-Identifier: MPL-2.0
// GENERATED by build.ts.
import { content, meta } from "../../${a.dir}/actor.ts";
import { runContent } from ${shell};
